...
// Text-color proxy. Light palettes flip these via window.TXT_RAW_OVERRIDE.
const TXT_DEFAULT = {
  primary: '#ecf6f8',
  muted:   '#7d9aa0',
  dim:     '#4a6770',
  inkOn:   '#040d0f',     // foreground when used on top of accent fills
};
const TXT = new Proxy({}, {
  get(_, k) {
...
// ─── relations: what's connected to what ──────────────────────────
// Used by the connection ray layer. Returns array of node ids.
function relatedNodes(selKey, data) {
  if (!selKey) return [];
  const [kind, id] = selKey.split(':');
  if (kind === 'svc') {
    // service → related strategies it runs
  ...
  if (kind === 'kb') {
    // kb source → strategies that consume it
    const m = {
  ...
  if (kind === 'str') {
    // strategy → sources it consumes + services it runs in
    const m = {
...
// ─── Ring labels (curved text along arc) ─────────────────────────
...
function KBRing({ sources, positions, hovered, setHovered, selected, setSelected, highlights }) {
  ...
  return (
    <g>
      ...
      {/* lane arc backgrounds — drawn in their *current* rotational frame */}
      {laneArcs.map(({ lane, startDeg, endDeg }) => {
        // these arc paths sit on the ring; we sweep them with rotate via the same offset
        // applied to children. But here we only want the static lane band.
        // We'll just draw it static; rotation reads through the node positions, not the band.
        return (
          <path key={lane}
...
function YouTubeStrand({ positions, hovered, setHovered, selected, setSelected, highlights }) {
  ...
  return (
    <g>
      ...
      {/* decade ticks — drawn at the *current* tick position so they orbit with the strand */}
      {sorted.map((d) => {
...
// ─── Connection rays — selected/highlighted nodes to their peers ─
function Connections({ positions, selected, highlights }) {
  const rays = useMemo(() => {
    const out = [];
    // From selected → its related set (bright, drawn)
    if (selected) {
      const sel = positions[selected];
      if (sel) {
        const rel = relatedNodes(selected, window.HC);
        rel.forEach(k => {
          const t = positions[k];
          if (t) out.push({ key: `s-${selected}-${k}`, x1: sel.x, y1: sel.y, x2: t.x, y2: t.y, strong: true });
        });
        // selected node also gets a tether to core
        out.push({ key: `s-${selected}-core`, x1: sel.x, y1: sel.y, x2: CX, y2: CY, strong: true });
      }
    }
    // From core → highlighted set (when ask response active)
...
// ─── Edge particles — trades flowing from strategy → core ────────
// Each strategy emits a particle every `cadence` seconds while it has open
// positions or today_trades > 0. Particles travel from the strategy's
// current world position into the core, fading as they go.
function ParticlesLayer({ data, positions, enabled }) {
  const partsRef = useRef([]);   // {id, fromKey, t (0..1), color, life}
  const tickRef = useRef(0);
  const lastEmitRef = useRef({}); // perStrategy: last emit time

  useAnimationFrame(useCallback((dt, now) => {
    if (!enabled) { partsRef.current = []; return; }
    tickRef.current = now;
    // Emit: each active strategy emits at a cadence proportional to today_trades
  ...
  // We need to re-render at ~30fps to animate; piggyback on a tick state
...
// ─── Camera focus — eased zoom toward selected node ──────────────
function useCamera(selected, positions) {
  ...
  useAnimationFrame(useCallback((dt) => {
    setCam(prev => {
      ...
      const k = 1 - Math.exp(-dt * 4.2); // critical-ish ease
      return {
...
function ControlCenter({ tweaks }) {
  ...
  const [askState, setAskState] = useState('idle'); // idle | thinking | answered

  // Rotation phase (deg, ticks over time)
  ...
  // highlights = ids highlighted by the current ask response
  ...
  // Wrap the whole scene in a translate+scale-around-center transform
  ...
  return (
    <>
      <div className="stage" onClick={() => setSelected(null)}>
        <svg viewBox={`0 0 ${VW} ${VH}`} preserveAspectRatio="xMidYMid meet">
          ...
          <g transform={camTransform}>
            ...
            <Connections positions={positions} selected={selected} highlights={highlights} />
      ...
      <window.NeuralAsk.AskGateway
        data={data}
        askState={askState} setAskState={setAskState}
        askResponse={askResponse} setAskResponse={setAskResponse}
...
window.NeuralGraph = { ControlCenter };
