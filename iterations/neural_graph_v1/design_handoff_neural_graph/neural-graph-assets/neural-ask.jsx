...
const { useState: useAskState, useEffect: useAskEffect, useRef: useAskRef, useMemo: useAskMemo } = React;

// ─── canned matcher ───────────────────────────────────────────────
// Each rule: { match: regex|fn, build: (data) => answer }
// Answer shape: { text, highlights: [ids], data?, follow_ups: [strings], focus?: nodeId }
...
// ─── component ────────────────────────────────────────────────────
function AskGateway({ data, askState, setAskState, askResponse, setAskResponse, setSelected }) {
  const [query, setQuery] = useAskState('');
  const inputRef = useAskRef(null);

  // Status ticker only shown when idle (no answer surface up)
  const tickerLines = useAskMemo(() => [
  ...
  const [idx, setIdx] = useAskState(0);
  useAskEffect(() => {
  ...
  return (
    <>
      {/* The response surface — appears above the input bar */}
      {askState !== 'idle' && (
